...
async def _prepapre_chunks_and_sources(
    request: AskRequest,
    opensearch_client, 
    embeddings_service, 
    ...
    use_vector_search = request.use_hybrid and query_embedding is not None
    ...
    if not chunks: 
        ...
        logger.warning(
            "RAG retrieved 0 chunks (index=%s, document_count=%s, search_total=%s, hybrid=%s). "
            "If document_count is 0, run your ingestion pipeline (e.g. Airflow DAGs) to index papers.",
...
async def ask_question(
    ...
    with rag_tracer.trace_rag_request("api_user", request.query) as trace: 
        try: 
            ...
            # Retrieve chunks
            chunks, sources, _ = await _prepapre_chunks_and_sources(
            ...
            # Generate answer
            llm_model = LLMClient.effective_rag_model(settings, request.model)
            with rag_tracer.trace_llm_generation(trace, llm_model, final_prompt) as gen_span:
                ...
                rag_tracer.end_generation(gen_span, answer, llm_model)
                ...
                with rag_tracer.trace_llm_generation(trace, llm_model, final_prompt) as gen_span:
                    ...
                    async for chunk in llm_client.generate_rag_answer_stream(
                        ...
                        if chunk.get("done", False):
                            rag_tracer.end_generation(gen_span, full_response, llm_model)
