...
from ..dependencies import AppSettingsDep, DatabaseDep, LLMDep, OpenSearchDep
...
async def health_check(
    ...
    # Same LLM client as /ask (app.state — not a fresh Settings() instance)
    try:
        llm_health = await llm_client.health_check()
        label = llm_health.get("provider", "llm")
        msg = llm_health["message"]
        if llm_health.get("version") and llm_health["version"] != "cloud":
            msg = f"{msg} (v{llm_health['version']})"
        services["llm"] = ServiceStatus(status=llm_health["status"], message=f"{label}: {msg}")
