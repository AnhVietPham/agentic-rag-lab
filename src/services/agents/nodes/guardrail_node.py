...
def continue_after_guardrail(state: AgentState, runtime: Runtime[Context]) -> Literal["continue", "out_of_scope"]:
    ...
    if not guardrail_result:
        logger.warning("No guardrail result found, routing to out_of_scope")
        return "out_of_scope"
    
    score = guardrail_result.score
    threshold = runtime.context.guardrail_threshold

    logger.info(f"Guardrail result: score={score}, threshold={threshold}")
