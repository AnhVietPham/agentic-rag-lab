...
from .index_config_hybrid import RAG_CHUNKS_MAPPING, HYBRID_RRF_PIPELINE
...
class OpenSearchClient:
    ...
    def get_index_stats(self) -> Dict[str, Any]: 
        """Get statistics for hybrid index."""
    ...
    def _create_hybrid_index(self, force: bool = False) -> bool: 
        ...
        try: 
            ...
            if not self.client.indices.exists(index=self.index_name):
                self.client.indices.create(index=self.index_name, body=RAG_CHUNKS_MAPPING)
    ...
    def bulk_index_chunks(self, chunks: List[Dict[str, Any]]) -> Dict[str, int]: 
        ...
        try: 
            ...
            logger.info(f"Buld indexed {success} chunks, {len(failed)} failed")
